...
Simulation::Simulation()
{
    ...
    branchingRatePerSegment = 3.5*0.5*1.0*0.3 *(1.0/tau);
...
void Simulation::readParameterFile(std::string fileName)
{
    ...
    double updateGridTime = 0.05*tau;
...
{   
    ...
    for(int f = 0; f < numBeads; f++)
    {
        ...
        if(filamentBondsInBondList && priorTag > -1)
        {
            ...
            newBond.destructionTime = 1000000.0*tau;
            ...
            for(int loopI = 0; loopI < pinfo.getNumParticles()-1-f; loopI++)
            {
                ...
                if(distMag >= minCrosslinkBondDist && distMag <= maxCrosslinkBondDist)
                {
                    // peramanent bond
                    {
                        ...
                        newBond.destructionTime = currSimTime + 10.0*tau;
        ...
        if(!skipBead && pinfo.checkTagExists(bondedTag))
        {
            ...
            if(bondedFilamentTag == filamentTag && randomRoll < Rdt)
            {
                ...
                if(new_filament_tag != -1)
                {
                    ...
                    newBond.creationTime = currSimTime;
                    newBond.destructionTime = 100000.0*tau;
...
void Simulation::capFilaments(double time_between_attempts)
{
	#pragma omp parallel for
	for(int f = 0; f < filaments.size(); f++)
	{
		if(filaments[f].getCappedBarbedEnd() == false)
		{
			std::mt19937& threadEngine = generators[omp_get_thread_num()];  
			...
			double rate = 0.757 / tau;
			
			double Rdt = rate * time_between_attempts;
			
			//try capping now based on rate
			int barbedEndTag = filaments[f].getTagAtIndex(numParticlesInFilament-1);
			Coordinate barbedEndPosition = pinfo.getPos(barbedEndTag);
			
			Coordinate displacementFromNucleatingBead = simBox.calcDisplacement(barbedEndPosition, pinfo.getPos(0));
			double distFromNucleatingBead = displacementFromNucleatingBead.getMagnitude();
			...
			Rdt = rate_multiplier * Rdt;
		
			/*if((distFromNucleatingBead - nucleatorBeadRadius) < 0.025)
			{
				//slower capping due to myosin
				if(randVal < 1.0 - exp(-Rdt * 0.75))
				{
					filaments[f].setCappedBarbedEnd(true);
				}
			}
			else*/
			{
...
void Simulation::run()
{
    ...
    double timeBetweenAddFilaments = 0.01*tau;
    ...
    timeBetweenBranchingEvents = 0.001*tau;
    ...
    double time_between_denovo = 1.0 / scaled_denovo_rate_real;
    int timestep_denovo = (int)round(time_between_denovo / dt);
    time_between_denovo = timestep_denovo * dt;
    
	double time_between_cap_attempt = dt;
	int timestep_between_cap_attempt = (int)round(time_between_cap_attempt / dt);
    time_between_cap_attempt = timestep_between_cap_attempt * dt;
    ...
    for(int mainLoopVar = 0; mainLoopVar < numSteps; mainLoopVar++)
    {
		...
		if(mainLoopVar % timestep_between_cap_attempt == 0)
		{
			capFilaments(time_between_cap_attempt);
		}
